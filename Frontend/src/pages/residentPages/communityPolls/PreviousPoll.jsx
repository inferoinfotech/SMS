        ...
        <div className="flex flex-wrap gap-5 items-start mt-5 max-md:max-w-full">
          {polls.map((poll, index) => {
            switch (poll.pollType) {
              case "Rating":
                return <RatingPollCard key={index} poll={poll} readOnly={true} />;
              case "Numeric":
                return <NumberPollCard key={index} poll={poll} readOnly={true} />;
              case "Text":
                return <TextPollCard key={index} poll={poll} readOnly={true} />;
              case "Multichoice":
                return <PollCard key={index} poll={poll} readOnly={true} />;
              default:
                return <PollCard key={index} poll={poll} readOnly={true} />;
            }
          })}
